...
class _ParentsPageState extends State<ParentsPage> {
  ...
  void _onSearchChanged() {
    _filterParents(_searchController.text);
  }

  void _filterParents(String searchTerm) {
    if (searchTerm.isEmpty) {
      setState(() {
        filteredParents = List.from(parents);
      });
      return;
    }
    ...
    setState(() {
      filteredParents = parents.where((parent) {
        return (parent['nom']?.toString().toLowerCase().contains(term) ?? false) ||
               (parent['email']?.toString().toLowerCase().contains(term) ?? false) ||
               (parent['telephone']?.toString().toLowerCase().contains(term) ?? false);
      }).toList();
    });
  }
  ...
  Future<void> deleteParent(int id) async {
    try {
      ...
      if (response.statusCode == 204) {
        fetchParents();
        _showSuccessSnackBar(context.tr('parents.delete_success'));
      } else {
              ...
              Padding(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    hintText: context.tr('parents.search_hint'),
          ...
          children: [
            ...
            Text(
              _searchController.text.isEmpty
                  ? context.tr('parents.no_parents')
                  : context.tr('parents.no_results'),
            ...
            if (_searchController.text.isNotEmpty)
              TextButton(
                ...
                child: Text(context.tr('parents.clear_search')),
              ),
          ],
        ),
      );
